...
public final class ResourceURL {
  ...
  /**
   * Returns true if the URL passed to it corresponds to a directory. This is slightly tricky due to
   * some quirks of the {@link JarFile} API. Only jar:// and file:// URLs are supported.
   *
    ...
    switch (protocol) {
      case "jar":
        try {
          ...
          // It turns out that JarEntry#isDirectory() really just tests whether the filename ends in
          // a '/'. If you try
          // to open the same URL without a trailing '/', it'll succeed - but the result won't be
          // what you want.
          // We try to get around this by calling getInputStream() on the file inside the jar. This
          // seems to return null
          // for directories (though that behavior is undocumented as far as I can tell). If you
          // have a better idea,
          // please improve this.
  ...
  /**
   * Appends a trailing '/' to a {@link URL} object. Does not append a slash if one is already
   * present.
   *
  ...
  /**
   * Returns the last modified time for file:// and jar:// URLs. This is slightly tricky for a
   * couple of reasons: 1) calling getConnection on a {@link URLConnection} to a file opens an
   * {@link InputStream} to that file that must then be closed - though this is not true for {@code
   * URLConnection}s to jar resources 2) calling getLastModified on {@link JarURLConnection}s
   * returns the last modified time of the jar file, rather than the file within
   *
