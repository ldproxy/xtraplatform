    ...
    public TaskQueue createQueue(String id) {
        return new TaskQueue() {
            ...
            public synchronized CompletableFuture<TaskStatus> launch(Task task) {
                ...
                if (Objects.nonNull(runningTask) && Objects.equals(runningTask.getId(), task.getId()) && Objects.equals(runningTask.getLabel(), task.getLabel())) {
                    LOGGER.debug("Ignoring task {} for {}, already running", task.getLabel(), task.getId());
                    return CompletableFuture.failedFuture(new IllegalArgumentException());
                }
                if (getFutureTasks().stream()
                                    .anyMatch(futureTask -> Objects.equals(futureTask.getId(), task.getId()) && Objects.equals(futureTask.getLabel(), task.getLabel()))) {
            ...
            public void remove(Task task) {
                List<Pair<Task, CompletableFuture<TaskStatus>>> toRemove = queue.stream()
            ...
            private synchronized void checkQueue(boolean currentIsDone) {
                ...
                if (Objects.isNull(currentTask) || currentTask.isDone()) {
                    ...
                    if (Objects.nonNull(task)) {
                        task.getLeft()
                            .logContext();
                        LOGGER.info("{} started", task.getLeft()
                                                      .getLabel());
                        ...
                        currentTask.onChange(
                                (progress, message) -> {
                                    if (LOGGER.isDebugEnabled()) {
                                        Thread.currentThread()
                                              .setName("bg-task-1");
                                        LOGGER.debug("{}: {} [{}]", task.getLeft()
                                                                        .getLabel(), message, new DecimalFormat("#%").format(progress));
                                    }
                                }, 1000);
                        currentTask.onDone(
                                (optionalThrowable) -> {
                                    ...
                                    if (optionalThrowable.isPresent()) {
                                        ...
                                        if (Objects.isNull(msg1)) {
                                            msg1 = String.format("%s at %s", throwable.getClass()
                                                                                      .getSimpleName(), throwable.getStackTrace()[0].toString());
                                        }
                                        String msg2 = Objects.nonNull(throwable.getCause()) ? throwable.getCause()
                                                                                                       .getMessage() : "";
                                        if (Objects.isNull(msg2)) {
                                            msg2 = String.format("%s at %s", throwable.getCause()
                                                                                      .getClass()
                                                                                      .getSimpleName(), throwable.getCause()
                                                                                                                 .getStackTrace()[0].toString());
                                        }

                                        LOGGER.error("{} failed: {} Cause: {}", task.getLeft()
                                                                                     .getLabel(), msg1, msg2);
                                        ...
                                        LOGGER.info("{} finished in {}", task.getLeft()
                                                                             .getLabel(), time);
                                    }
