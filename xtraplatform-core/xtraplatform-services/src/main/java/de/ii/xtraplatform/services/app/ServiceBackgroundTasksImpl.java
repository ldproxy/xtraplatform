...
@Wbp(
        filter =
                Registry.FILTER_PREFIX + ServiceBackgroundTasksImpl.SERVICE_BACKGROUND_TASK + Registry.FILTER_SUFFIX,
        onArrival = Registry.ON_ARRIVAL_METHOD,
        onDeparture = Registry.ON_DEPARTURE_METHOD)
public class ServiceBackgroundTasksImpl implements ServiceBackgroundTasks, Registry<ServiceBackgroundTask<? extends Service>> {

    private static final Logger LOGGER = LoggerFactory.getLogger(ServiceBackgroundTasksImpl.class);
    static final String SERVICE_BACKGROUND_TASK = "de.ii.xtraplatform.services.domain.ServiceBackgroundTask";
    ...
    ServiceBackgroundTasksImpl(@Context BundleContext context, @Requires Scheduler scheduler, @Requires EntityRegistry entityRegistry) {
    ...
    private <T extends Service> void onServiceStop(T service) {
        ...
        commonQueue.getFutureTasks().stream().filter(task -> Objects.equals(task.getId(), service.getId())).forEach(commonQueue::remove);
    }

    private <T extends Service, U extends Service> void scheduleIfMatching(T service, ServiceBackgroundTask<U> task) {
    ...
    private <T extends Service> void schedule(T service, ServiceBackgroundTask<T> task) {
        ...
        if (task.runPeriodic(service).isPresent()) {
            //LOGGER.debug("SCHEDULE TASK {} FOR {} WITH {}", task.getLabel(), service.getId(), task.runPeriodic(service).get());
        }
    }
