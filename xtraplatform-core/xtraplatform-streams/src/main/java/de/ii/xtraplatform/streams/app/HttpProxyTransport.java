...
public class HttpProxyTransport extends ClientTransport {
  ...
  public HttpProxyTransport(InetSocketAddress proxyAddress, HttpCredentials proxyCredentials) {
  ...
  @Override
  public Flow<ByteString, ByteString, CompletionStage<OutgoingConnection>> connectTo(
      String host, int port, ClientConnectionSettings settings, ActorSystem system) {

    Flow<ByteString, ByteString, NotUsed> identity =
        Flow.fromFunction((Function<ByteString, ByteString>) byteString -> byteString);
    Flow<ByteString, ByteString, CompletionStage<OutgoingConnection>> connect =
        underlyingTransport.connectTo(
            proxyAddress.getHostString(), proxyAddress.getPort(), settings, system);
    Function<CompletionStage<OutgoingConnection>, CompletionStage<OutgoingConnection>>
        rewriteOutgoingConnection =
            completionStage ->
                completionStage.thenApply(
                    outgoingConnection ->
                        new OutgoingConnection(
                            new akka.http.scaladsl.Http.OutgoingConnection(
                                outgoingConnection.localAddress(),
   ...
   * <p>after:
   *
   * <p>GET http://icanhazip.com:80/ HTTP/1.1 Proxy-Connection: Keep-Alive Proxy-Authorization:
   * Basic cGF1bGdob3N0OmNtODM1Ng== ...
   */
  private Flow<ByteString, ByteString, ?> rewriteHeader(String host, int port) {

    return Flow.fromFunction(
        byteString -> {
          Tuple2<ByteString, ByteString> span =
              byteString.span(byt -> !Objects.equals(byt, "\n".getBytes()[0]));
          String withFullPath =
