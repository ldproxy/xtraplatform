package de.ii.xtraplatform.store.domain.entities.maptobuilder.encoding;
...
class BuildableMapEncoding<T extends Buildable<T>, U extends BuildableBuilder<T>> {
    @Encoding.Impl
    private BuildableMap<T, U> field;
    ...
    static class Builder<T extends Buildable<T>, U extends BuildableBuilder<T>> { // <-- copy type parameters from the encoding
        ...
        BuildableMap<T, U> build() {
            ...
            return new ImmutableBuildableMap.Builder<T, U>().builders(builderMap)
                                                            .build();
        }
    }
}
