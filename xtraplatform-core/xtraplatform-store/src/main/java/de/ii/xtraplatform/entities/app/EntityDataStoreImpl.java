...
@Provides
@Instantiate
public class EntityDataStoreImpl extends AbstractMergeableKeyValueStore<EntityData> implements EntityDataStore<EntityData> {
    ...
    protected EntityDataStoreImpl(@Requires EventStore eventStore, @Requires Jackson jackson,
    ...
    private List<MutationEvent> processEvent(MutationEvent event) {
        ...
        if (!overridesPath.getKeyPath()
                          .isEmpty()) {
            Optional<KeyPathAlias> keyPathAlias = entityFactory.getKeyPathAlias(overridesPath.getKeyPath()
